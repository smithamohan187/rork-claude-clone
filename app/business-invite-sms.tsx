...
import { ArrowLeft, Circle, Eye, Pencil, Search, Send, ShieldCheck } from 'lucide-react-native';
...
export default function BusinessInviteSmsScreen() {
  ...
  const defaultMsg = useMemo(
    () =>
      `Hey! I think your business would be a great fit for TouchPoint. It's a platform where businesses connect with loyal customers through offers, events, and rewards.`,
    [],
  );
  ...
  const loadContacts = useCallback(async () => {
    try {
      ...
      console.log('[BizInviteSms] contacts error', e);
      setContacts(fallback);
      setPermissionDenied(true);
    }
  }, [fallback]);
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      console.log('[BizInviteSms] send error', e);
      showSnackbar('Could not send SMS');
    }
  }, [selected, contacts, fullMessage, goExplore, showSnackbar]);

  return (
    <View style={styles.container} testID="biz-invite-sms-screen">
              ...
              filtered.map((item) => {
                ...
                return (
                  <Pressable
                    ...
                    testID={`biz-sms-contact-${item.id}`}
                  >
                    {isSelected ? (
