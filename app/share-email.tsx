...
export default function ShareEmailScreen() {
  ...
  const defaultBody = useMemo(
    () =>
      `Hi there,\n\nI thought you'd love this on TouchPoint! It's a platform where you can discover local businesses, earn reward points, and get exclusive deals in your area.\n\nThis one is from ${authorName} — take a look!`,
    [authorName],
  );

  const [emailInput, setEmailInput] = useState<string>('');
  const [emails, setEmails] = useState<string[]>([]);
  const [subject, setSubject] = useState<string>('Check this out on TouchPoint!');
  const [body, setBody] = useState<string>(defaultBody);

  const previewBody = useMemo(() => `${body.trim()}\n\nCheck it out here:\n${shareUrl}`, [body, shareUrl]);
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      console.log('[ShareEmail] send error', e);
      showSnackbar('Could not open email');
    }
  }, [emails, emailInput, subject, previewBody, router, showSnackbar]);
  ...
  return (
    <View style={styles.container} testID="share-email-screen">
      <SafeAreaView edges={['top']} style={styles.safeTop}>
        <View style={styles.header}>
          <Pressable onPress={() => router.back()} style={styles.iconBtn} hitSlop={8} testID="share-email-back">
            <ArrowLeft size={20} color={Colors.bannerText} />
          </Pressable>
          <Text style={styles.headerTitle}>Share via Email</Text>
          <Pressable onPress={closeAndGoHome} style={styles.iconBtn} hitSlop={8} testID="share-email-close">
        ...
        >
          ...
          <View style={styles.previewCard}>
            ...
            <View style={{ flex: 1, paddingVertical: 12, paddingRight: 12 }}>
              <Text style={styles.previewSubject} numberOfLines={2}>{subject}</Text>
